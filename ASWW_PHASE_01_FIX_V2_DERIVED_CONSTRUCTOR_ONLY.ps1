...
function Stop-Gate([string]$Reason, [int]$Code = 1) {
    Write-Host ""
    Write-Host "PHASE_01_V2_CONSTRUCTOR_FIX=STOPPED" -ForegroundColor Red
...
foreach ($Required in @($BuildScript,$BaseH,$V2H,$V2Cpp)) {
...
Write-Host " BASE/V2 CONSTRUCTOR AUDIT" -ForegroundColor Cyan
Write-Host "============================================================" -ForegroundColor Cyan

$BaseHasObjectInit = $BaseHeader -match 'AASCharacter\s*\(\s*const\s+FObjectInitializer\s*&'
...
Write-Host "BASE_HAS_OBJECTINITIALIZER_CONSTRUCTOR=$BaseHasObjectInit"
...
if (-not $BaseHasObjectInit) {
    Stop-Gate "BASE_CLASS_OBJECTINITIALIZER_CONSTRUCTOR_NOT_FOUND" 12
}

# The compile error C2512 is caused by V2 trying to call a nonexistent default
# AASCharacter constructor. We only need to fix the derived constructor chain.
# Do not edit ASCharacter here.
$Stamp = Get-Date -Format "yyyyMMdd_HHmmss"
$BackupRoot = Join-Path $ProjectRoot "Saved\Verification\Phase01V2CtorOnly_$Stamp"
New-Item -ItemType Directory -Force -Path $BackupRoot | Out-Null

Copy-Item -LiteralPath $V2H -Destination (Join-Path $BackupRoot "ASPlayerCharacterV2.h.before") -Force
Copy-Item -LiteralPath $V2Cpp -Destination (Join-Path $BackupRoot "ASPlayerCharacterV2.cpp.before") -Force
Write-Host "BACKUP_ROOT=$BackupRoot"

if ($V2HeaderObjInit -eq 0) {
    if ($V2HeaderNoArg -ne 1) {
        Stop-Gate "V2_HEADER_CONSTRUCTOR_PATTERN_UNEXPECTED" 20
    }
...
elseif ($V2HeaderObjInit -ne 1) {
    Stop-Gate "V2_HEADER_MULTIPLE_OBJECTINITIALIZER_CONSTRUCTORS_$V2HeaderObjInit" 21
}

if ($V2CppObjInit -eq 0) {
    if ($V2CppNoArg -ne 1) {
        Stop-Gate "V2_CPP_CONSTRUCTOR_PATTERN_UNEXPECTED" 22
    }
...
elseif ($V2CppObjInit -eq 1) {
    $HasSuperObjectInit = $V2Source -match 'AASPlayerCharacterV2::AASPlayerCharacterV2\s*\(\s*const\s+FObjectInitializer\s*&[^)]*\)\s*:\s*Super\s*\(\s*ObjectInitializer\s*\)'
    if (-not $HasSuperObjectInit) {
        Stop-Gate "V2_OBJECTINITIALIZER_CONSTRUCTOR_EXISTS_BUT_SUPER_CHAIN_NOT_VERIFIED" 23
    }
}
else {
    Stop-Gate "V2_CPP_MULTIPLE_OBJECTINITIALIZER_CONSTRUCTORS_$V2CppObjInit" 24
}
...
Write-Host ""
Write-Host "============================================================" -ForegroundColor Cyan
Write-Host " POST-PATCH V2 VERIFICATION" -ForegroundColor Cyan
Write-Host "============================================================" -ForegroundColor Cyan

$V2HeaderDisk = Get-Content -Raw -LiteralPath $V2H
$V2SourceDisk = Get-Content -Raw -LiteralPath $V2Cpp

$HeaderPass = $V2HeaderDisk -match 'AASPlayerCharacterV2\s*\(\s*const\s+FObjectInitializer\s*&'
$SourcePass = $V2SourceDisk -match 'AASPlayerCharacterV2::AASPlayerCharacterV2\s*\(\s*const\s+FObjectInitializer\s*&[^)]*\)\s*:\s*Super\s*\(\s*ObjectInitializer\s*\)'

Write-Host "V2_HEADER_OBJECTINITIALIZER=PASS_$HeaderPass"
Write-Host "V2_CPP_SUPER_OBJECTINITIALIZER=PASS_$SourcePass"

if (-not ($HeaderPass -and $SourcePass)) {
    Stop-Gate "POST_PATCH_V2_CONSTRUCTOR_VERIFICATION_FAILED" 25
}
...
if (Test-Path -LiteralPath $StageRoot) {
    $OldStage = "$StageRoot`_PRE_PHASE01_V2_CTOR_$Stamp"
    try {
        Move-Item -LiteralPath $StageRoot -Destination $OldStage
        Write-Host "OLD_STAGE_MOVED=$OldStage"
    }
    catch {
        Stop-Gate "STAGE_ROOT_LOCKED_$($_.Exception.Message)" 28
    }
}

$EvidenceRoot = Join-Path $ProjectRoot "Saved\RuntimeEvidence\Phase01V2CtorOnly"
New-Item -ItemType Directory -Force -Path $EvidenceRoot | Out-Null
$PackageLog = Join-Path $EvidenceRoot "phase01_v2_ctor_package_$Stamp.log"
...
Write-Host ""
Write-Host "PHASE_01_V2_CONSTRUCTOR_FIX=PASS" -ForegroundColor Green
Write-Host "C2512_DERIVED_CONSTRUCTOR_CHAIN=FIXED" -ForegroundColor Green
Write-Host "BASE_ASCHARACTER=NOT_MODIFIED_BY_THIS_SCRIPT" -ForegroundColor Green
