...
$PreexistingTPMarker = $Original -match "ASWW_TP_RIFLE_TACTICAL_BEGIN"
$PreexistingTPABP = $Original -match "/Game/Variant_Shooter/Anims/ABP_TP_Rifle"
Write-Host "SOURCE_ALREADY_HAS_TP_RIFLE_TACTICAL_MARKER=$PreexistingTPMarker"
Write-Host "SOURCE_ALREADY_HAS_TP_RIFLE_ABP_PATH=$PreexistingTPABP"

if ($PreexistingTPMarker -or $PreexistingTPABP) {
    Stop-Gate "SOURCE_ALREADY_PARTIALLY_OR_FULLY_PATCHED_REVIEW_BEFORE_RERUN" 141
}
...
$TransformPattern = 'RifleVisual->SetRelativeLocation\([^;]+;\s*RifleVisual->SetRelativeRotation\([^;]+;\s*RifleVisual->SetRelativeScale3D\([^;]+;'
$TransformRegex = [regex]::new(
    $TransformPattern,
    [System.Text.RegularExpressions.RegexOptions]::Singleline
)
$TransformMatchCount = $TransformRegex.Matches($Block).Count
Write-Host "RIFLE_LOCAL_TRANSFORM_BLOCK_COUNT=$TransformMatchCount"

if ($TransformMatchCount -eq 1) {
    $Block = $TransformRegex.Replace(
        $Block,
        'RifleVisual->SetRelativeTransform(FTransform::Identity);',
        1
    )
}
elseif ($Block -match 'RifleVisual->SetRelativeTransform\(FTransform::Identity\);') {
    Write-Host "RIFLE_LOCAL_TRANSFORM_ALREADY_IDENTITY=True"
}
else {
    Stop-Gate "EXPECTED_ONE_RIFLE_LOCAL_TRANSFORM_BLOCK_FOUND_$TransformMatchCount" 421
}
