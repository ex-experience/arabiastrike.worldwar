...
if ($DestExisted) {
    $ExistingDestFileCount = @(
        Get-ChildItem -LiteralPath $DestRifleRoot -Recurse -File -ErrorAction SilentlyContinue
    ).Count
}
Write-Host "DEST_RIFLE_ROOT_PREEXISTED=$DestExisted"
Write-Host "DEST_RIFLE_ROOT_EXISTING_FILE_COUNT=$ExistingDestFileCount"

if ($DestExisted -and $ExistingDestFileCount -gt 0) {
...
elseif ($DestExisted) {
    Write-Host "EXISTING_RIFLE_ASSET_BACKUP=NOT_NEEDED_EMPTY_DIR_FROM_PRIOR_STOP"
}
else {
...
# IMPORTANT:
# The previous version used Copy-Item -LiteralPath with a "*" wildcard.
# -LiteralPath never expands wildcards, so nothing was copied.
# Copy each real child explicitly instead.
$SourceChildren = @(
    Get-ChildItem -LiteralPath $SourceRifleRoot -Force -ErrorAction Stop
)

Write-Host "SOURCE_RIFLE_CHILD_COUNT=$($SourceChildren.Count)"

if ($SourceChildren.Count -eq 0) {
    Stop-Gate "SOURCE_RIFLE_ROOT_IS_EMPTY" 19
}
...
$CopiedFiles = @(
    Get-ChildItem -LiteralPath $DestRifleRoot -Recurse -File -ErrorAction Stop
)

$CopiedCount = @(
    $CopiedFiles | Where-Object { $_.Extension -ieq ".uasset" }
).Count

Write-Host "COPIED_RIFLE_FILE_COUNT=$($CopiedFiles.Count)"
...
$ExpectedCopiedSKRifle = Join-Path $DestRifleRoot "Meshes\SK_Rifle.uasset"
Write-Host "COPIED_SK_RIFLE_EXISTS=$(Test-Path -LiteralPath $ExpectedCopiedSKRifle -PathType Leaf)"

if (-not (Test-Path -LiteralPath $ExpectedCopiedSKRifle -PathType Leaf)) {
    Stop-Gate "EXPECTED_COPIED_SK_RIFLE_NOT_FOUND" 201
}
