...
$PreTelemetryTags = ([regex]::Matches($Original, "ASWW_Telemetry_")).Count
$PreMoveTags = ([regex]::Matches($Original, "ASWW_MoveState_")).Count
...
Write-Host "============================================================" -ForegroundColor Cyan
Write-Host "ASWW_TELEMETRY_LITERAL_LOG_COUNT=$PreTelemetry"
Write-Host "ASWW_TELEMETRY_TAG_COUNT=$PreTelemetryTags"
Write-Host "ASWW_MOVE_STATE_LOG_COUNT=$PreMoveState"
Write-Host "ASWW_MOVESTATE_TAG_COUNT=$PreMoveTags"
Write-Host "TEMP_VISUAL_PROOF_COUNT=$PreVisual"
Write-Host "ASWW_REAL_PLAYER_MANNY_COUNT=$PreManny"

# There are 7 literal uppercase log markers:
# CHARACTER_BEGIN, CONTROLLER_SEEN, INPUT_SETUP,
# AXIS_MOVEFORWARD, AXIS_MOVERIGHT, AXIS_TURN, AXIS_LOOKUP.
# The earlier audit reported 17 because PowerShell -match is case-insensitive
# and also counted 10 mixed-case ASWW_Telemetry_* tag references.
if ($PreTelemetry -ne 7) {
    Stop-Gate "UNEXPECTED_LITERAL_ASWW_TELEMETRY_COUNT_$PreTelemetry" 12
}
if ($PreMoveState -ne 5) {
    Stop-Gate "UNEXPECTED_ASWW_MOVE_STATE_COUNT_$PreMoveState" 13
}
if ($PreTelemetryTags -ne 10) {
    Stop-Gate "UNEXPECTED_ASWW_TELEMETRY_TAG_COUNT_$PreTelemetryTags" 131
}
if ($PreMoveTags -ne 4) {
    Stop-Gate "UNEXPECTED_ASWW_MOVESTATE_TAG_COUNT_$PreMoveTags" 132
}
if ($PreVisual -ne 0) {
