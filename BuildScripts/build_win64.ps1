...
$ArchiveRoot = Join-Path $ProjectRoot "BuildOutput\Client"
...
if ($LASTEXITCODE -ne 0) {
    Write-Output "WIN64_PACKAGE_RESULT=FAIL_EXIT_$LASTEXITCODE"
    exit $LASTEXITCODE
}

$Artifacts = @(Get-ChildItem -LiteralPath $ArchiveRoot -Recurse -File -Filter "ArabiaStrikeWorldWar.exe" -ErrorAction SilentlyContinue |
    Where-Object { $_.FullName -notmatch '\\Engine\\Binaries\\' })
if ($Artifacts.Count -eq 0) {
    throw "UAT completed but no packaged ArabiaStrikeWorldWar.exe was found under '$ArchiveRoot'."
}

$Artifacts | ForEach-Object { Write-Output "WIN64_CLIENT_ARTIFACT=$($_.FullName)" }
Write-Output "WIN64_PACKAGE_RESULT=PASS"
exit 0
