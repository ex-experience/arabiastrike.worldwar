...
function Save-Checkpoint([string]$Label) {
    ...
    & git -C $ProjectRoot diff --binary | Set-Content -LiteralPath (Join-Path $Root "working_tree.patch") -Encoding UTF8
    & git -C $ProjectRoot diff --cached --binary | Set-Content -LiteralPath (Join-Path $Root "index.patch") -Encoding UTF8
    & git -C $ProjectRoot diff --stat | Set-Content -LiteralPath (Join-Path $Root "diff_stat.txt") -Encoding UTF8
    ...
    $Changed += @(& git -C $ProjectRoot diff --cached --name-only)
